<template>
  <BaseMain>
    <UiIntro
      title="Aktionen"
      intro="Hier findest Du alle unsere Aktionen auf einen Blick."
    />
    <div
      v-if="results.length"
...
const { results: articles } = await useApiData<API.Kaninchen.Article[]>(
  "/articles",
  {
    key: "articles-aktionen",
    params: { limit: "50", sort: "-createdAt", "_meta.sale": true },
  }
);
...
const { results: products } = await useApiData<API.Kaninchen.Product[]>(
  "/products",
  {
    key: "products-aktionen",
    params: { limit: "50", sort: "-createdAt", "_meta.sale": "true" },
  }
);

const results = computed(() => {
  return [...articles.value, ...products.value];
});
